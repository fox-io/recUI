...
raid_threat = {}
local display_frame = CreateFrame("Frame", "utility_threat_meter", UIParent)
...
local function MakeDisplay()
	...
	f:SetWidth(192.5)
	f:SetHeight(138)
	f:SetPoint("TOPLEFT", LRBoxPanel, "TOPLEFT", 5, -6)
	...
	f.titletext:SetFont(recMedia.fontFace.SMALL, 8, "THINOUTLINE")
	...
	for i=1,10 do
		...
		f.bars[i].lefttext:SetFont(recMedia.fontFace.SMALL, 8, "THINOUTLINE")
		...
		f.bars[i].righttext:SetFont(recMedia.fontFace.SMALL, 8, "THINOUTLINE")
	...
	display_frame:HookScript("OnSizeChanged", function(frame, ...)
		-- Truncate bars
		local bar_room
		bar_room = floor((utility_threat_meter:GetHeight()-40)/10)
		for i=1,10 do
			utility_threat_meter.bars[i]:Hide()
		end
		if bar_room > 0 then
			for i=1,bar_room do
				utility_threat_meter.bars[i]:Show()
			end
		end
	end)
end
